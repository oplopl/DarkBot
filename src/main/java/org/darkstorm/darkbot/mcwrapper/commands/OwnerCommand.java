...
public class OwnerCommand extends AbstractCommand {
	...
	public void execute(String[] args) {
		...
		case "add":
			...
			controller.say("/msg " + controller.getOwners()[0] + " Added owner " + args[1] + ".");
			break;
		case "remove":
			if(!controller.isOwner(args[1])) {
				controller.say("/msg " + controller.getOwners()[0] + " No such owner " + args[1] + ".");
			} else if(controller.getOwners().length == 1) {
				controller.say("/msg " + controller.getOwners()[0] + " Must have at least one owner.");
			} else {
				controller.removeOwner(args[1]);
				controller.say("/msg " + controller.getOwners()[0] + " Removed owner " + args[1]);
			}
			break;
		case "set":
			...
			controller.addOwner(args[1]);
			controller.say("/msg " + controller.getOwners()[0] + " Set owner to " + args[1] + ".");
		}
	}
}
