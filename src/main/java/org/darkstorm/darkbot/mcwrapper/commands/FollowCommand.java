	...
	public void execute(String[] args) {
		...
		for(Entity entity : bot.getWorld().getEntities()) {
			if(entity instanceof PlayerEntity && isFollowable(args, ((PlayerEntity) entity).getName())) {
				...
				controller.say("/msg " + ((PlayerEntity) entity).getName() + " Now following " + (args.length > 0 ? Util.stripColors(((PlayerEntity) entity).getName()) : "you") + ".");
				return;
			}
		}
		if(args.length > 0)
			controller.say("/msg " + controller.getOwners()[0] + " Player " + args[0] + " not found.");
		else
			controller.say("/msg " + controller.getOwners()[0] + " Owner not found.");
	}
